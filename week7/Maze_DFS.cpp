...
int n, m;
int graph[201][201];
// 이동할 네 가지 방향 정의 (상, 하, 좌, 우) int dx[] = {-1, 1, 0, 0};
int dy[] = {0, 0, -1, 1};
int main(void) {
    cin>>n>>m;
    for (int i = 0; i < n; i++) {
...
int dfs(int x, int y) {
    // 스택(Stack) 구현을 위해 stack 라이브러리 사용 stack<pair<int, int> > st;
    st.push({x, y});
    // 스택이 빌 때까지 반복하기
    while(!st.empty()) {
        int x = st.top().first;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int ny = y + dy[i];
        //미로 찾기 공간을 벗어난 경우 무시
        if (nx < 0 || nx >= n || ny < 0 || ny >= m) continue; // 벽인 경우 무시
        if (graph[nx][ny] == 0) continue;
            //해당 노드를 처음 방문하는 경우에만 최단 거리 기록 if (graph[nx][ny] == 1) {
